...
\begin{table}[H]\small
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{|p{3.2cm}|p{2.8cm}|p{5.2cm}|p{4.2cm}|}
            ...
            \textbf{Random Forest}                                                          & Bagging (Árboles)          &
            Bootstrap con selección aleatoria de atributos; reducción de varianza           &
            Robusto, estable y poco sensible a hiperparámetros                                                                                                                          \\
            \hline

            \textbf{Bagged Decision Trees (BaggedDT)}                                       & Bagging                    &
            Árboles sin poda entrenados sobre muestras bootstrap                            &
            Alta varianza individual; mejora por agregación                                                                                                                             \\
            \hline

            \textbf{XGBoost}                                                                & Boosting (GBM)             &
            Regularización L1/L2, segundo orden, manejo de valores faltantes                &
            Muy preciso; sensible a tuning                                                                                                                                              \\
            \hline

            \textbf{LightGBM}                                                               & Boosting (Leaf-wise)       &
            Crecimiento hoja-a-hoja, histogramas, muy eficiente                             &
            Rápido; riesgo de sobreajuste en datasets pequeños                                                                                                                          \\
            \hline

            \textbf{CatBoost}                                                               & Boosting especializado     &
            Codificación ordenada de categorías; robusto al ruido                           &
            Excelente rendimiento sin gran tuning                                                                                                                                       \\
            \hline

            \textbf{Gradient Boosting Decision Trees (GBDT)}                                & Boosting                   &
            Árboles secuenciales ajustados a residuos                                       &
            Buen rendimiento; coste computacional elevado                                                                                                                               \\
            \hline

            \textbf{AdaBoost}                                                               & Boosting (Stumps)          &
            Aumenta el peso de observaciones mal predichas                                  &
            Efectivo en problemas limpios, pero menos robusto                                                                                                                           \\
            \hline

            \textbf{K-Nearest Neighbors (KNN)}                                              & Basado en instancia        &
            Predicción por proximidad; no requiere entrenamiento                            &
            Sensible a escala y ruido; coste alto en predicción                                                                                                                         \\
            \hline

            \textbf{Multi-Layer Perceptron (MLP)}                                           & Red neuronal               &
            Capaz de capturar relaciones no lineales complejas                              &
            Requiere normalización y tuning de regularización                                                                                                                           \\
            \hline

            \textbf{Support Vector Regression (SVR)}                                        & Modelo lineal con márgenes &
            Modelo de gran margen; utiliza un kernel lineal ($\text{kernel}=\text{linear}$) &
            Robusto al sobreajuste; coste computacional elevado                                                                                                                         \\
            \hline

            \textbf{Bayesian Neural Network (BayesianNN)}                                   & Probabilístico             &
            Estimación bayesiana de parámetros; cuantifica incertidumbre                    &
            Reduce sobreajuste; mayor coste de entrenamiento                                                                                                                            \\
            \hline
        \end{tabular}
    }
